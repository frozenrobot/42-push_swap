...
int first_valid(t_stack *stack, int ac)
{
	int i;
	t_stack *temp;
...
void midpoint_algo(t_stack *stack_a, t_stack *stack_b, t_stack *stack_d, int ac)
{
	...
	else
	{
		...
		while (valid_count_a_top(stack_a, stack_d->valid, ac) > 2)
		{
			mid2 = midpoint_chunk_lessthan_a(stack_a, stack_d->valid, ac);
			push_b_iterate_top(stack_a, stack_b, ac, mid2);
		}
		midpoint_algo_two_a(stack_a, stack_d->valid, ac);
...
void sort(t_stack *stack_a, t_stack *stack_b, int ac)
{
	...
	{
		// while (valid_count_b_top(stack_b, stack_d->next->value, ac) != 0)
